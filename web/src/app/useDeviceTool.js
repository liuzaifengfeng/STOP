...
export function useDeviceTool() {
    ...
    const info = ref();
    const status = ref();
    const config = reactive({ alias: 'STOP-C6', statusPeriodMs: 5000, radioTxTimeoutMs: 1000 });
    const radioMode = ref('text');
    ...
    const signalStrength = computed(() => {
        ...
        if (rssi == null)
            return { label: '测量中', level: 'unknown' };
        if (rssi >= -55)
            return { label: '很强', level: 'excellent' };
        if (rssi >= -67)
            return { label: '较强', level: 'good' };
        if (rssi >= -75)
            return { label: '一般', level: 'fair' };
        return { label: '较弱', level: 'weak' };
    });
    const linkQuality = computed(() => {
        const metrics = linkMetrics.value;
        if (metrics.samples < 4)
            return { label: '采样中', level: 'unknown', score: null, title: `有效样本 ${metrics.samples}/4` };
    ...
    function stopLinkProbe() {
        if (linkProbeTimer !== undefined)
            window.clearInterval(linkProbeTimer);
    ...
    async function reconcileOtaResult(result) {
        ...
        if (result.transferId !== pending.transferId || result.imageSize !== pending.imageSize || result.sha256.toLowerCase() !== pending.sha256.toLowerCase())
            return;

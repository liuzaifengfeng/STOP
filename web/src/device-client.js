...
export class StopDeviceClient {
    ...
    get name() { return this.device?.name ?? 'STOP-C6'; }
    getLinkMetrics() {
    ...
    async connect() {
        ...
        if (!server)
            throw new Error('无法创建 BLE GATT 连接');
    ...
    nextRequestId() { this.requestId = (this.requestId % 0xffff) + 1; return this.requestId; }
    nextSequence() { this.sequence = (this.sequence + 1) >>> 0; return this.sequence; }
    recordLinkSample(success, rttMs) {
        this.linkSamples.push({ success, rttMs });
        if (this.linkSamples.length > 50)
            this.linkSamples.shift();
    }
    ...
    async request(type, payload = new Uint8Array(), timeoutMs = 8000) {
        if (!this.control || !this.connected)
            throw new Error('设备尚未连接');
        const requestId = this.nextRequestId();
        const response = this.waitForResponse(requestId, timeoutMs);
        try {
            await this.control.writeValueWithResponse(encodeFrame(type, FrameFlag.Request, requestId, this.nextSequence(), payload));
    ...
    async bulkRequest(payload) {
        if (!this.bulk || !this.connected)
            throw new Error('设备尚未连接');
        const requestId = this.nextRequestId();
        const response = this.waitForResponse(requestId, 10000);
        try {
            await this.bulk.writeValueWithoutResponse(encodeFrame(MessageType.OtaData, FrameFlag.Request, requestId, this.nextSequence(), payload));
            return await response;
        }
        catch (error) {
            const pending = this.pending.get(requestId);
    ...
    async setConfig(config) {
        ...
        payload.set(alias, offset + 5);
        offset += 5 + alias.length;
        view.setUint16(offset, 2, true);
        payload[offset + 2] = 2;
        view.setUint16(offset + 3, 4, true);
        view.setUint32(offset + 5, config.statusPeriodMs, true);
        offset += 9;
    ...
    async otaUpdate(image, version, onProgress, onAttempt) {
        ...
        if (beginStatus.chunkSize === 0)
            throw new Error('设备未返回有效 OTA 分块大小');
        ...
        while (offset < image.length) {
            ...
            data.set(chunk, 10);
            view.setUint32(10 + chunk.length, crc32(chunk), true);
        ...
        if (finalStatus.state !== 4)
            throw new Error(`设备未进入重启状态（OTA 状态 ${finalStatus.state}）`);
        return attempt;
    }
}
function parseConfig(payload) {
    ...
    for (let index = 0; index < payload[0]; index += 1) {
        if (offset + 5 > payload.length)
            throw new Error('参数 TLV 损坏');
        const key = view.getUint16(offset, true), type = payload[offset + 2], length = view.getUint16(offset + 3, true);
        offset += 5;
        if (offset + length > payload.length)
            throw new Error('参数 TLV 长度错误');
        if (key === 1 && type === 3)
            result.alias = bytesText(payload.subarray(offset, offset + length));
        else if (key === 2 && type === 2 && length === 4)
